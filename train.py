...
def L1_loss_appearance(image, gt_image, gaussians, view_idx, return_transformed_image=False):
    ...
    if not return_transformed_image:
        ...
        transformed_image = torch.nn.functional.interpolate(
            transformed_image, size=(origH, origW), mode="bilinear", align_corners=True
        )[0]
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    scene = Scene(dataset, gaussians, skip_test=True, use_rgb=dataset.use_rgb)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        render_pkg = render(
            viewpoint_cam, gaussians, pipe, background, kernel_size, require_coord=False, require_depth=True
        )
        ...
        rendered_depth_image: torch.Tensor = render_pkg["expected_depth"]
        ...
        if reg_kick_on:
            ...
            if require_depth:
                ...
                depth_middepth_normal = depth_double_to_normal(
                    viewpoint_cam, rendered_expected_depth, rendered_median_depth
                )
            else:
                rendered_expected_coord: torch.Tensor = render_pkg["expected_coord"]
                rendered_median_coord: torch.Tensor = render_pkg["median_coord"]
                rendered_normal: torch.Tensor = render_pkg["normal"]
                depth_middepth_normal = point_double_to_normal(
                    viewpoint_cam, rendered_expected_coord, rendered_median_coord
                )
            depth_ratio = 0.6
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[1] + list(range(0, 30_000, 1000)))
